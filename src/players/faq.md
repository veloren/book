...
> However, you can also use other apps if you feel like it (Goxel, etc) but some features we use might not be available.
...
> There is absolutely no drawback towards keeping an account, all the passwords are encrypted. We hash it once client-side with the auth domain as a salt using argon2id and then once again server-side with a randomized 256-bit salt.
> The client-side pre-hash is to prevent third party authentication servers from discovering potentially reused user passwords
